...
const FILTERS = ["Tous", "brouillon", "envoyé", "accepté"];
...
export default function DashboardProformas() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("factures").select("*").eq("user_id", user.id).eq("type_document", "Proforma")
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 14, marginBottom: 24 }}>
        {[
          { label: "Total proformas", val: String(docs.length), color: "#6366F1" },
          { label: "Montant total", val: `${fmt(total)} FCFA`, color: "#0A0F0D" },
          { label: "Envoyés", val: String(docs.filter(d => d.statut === "envoyé").length), color: "#3B82F6" },
        ].map(s => (
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 16, alignItems: "center", justifyContent: "space-between" }}>
        ...
        <Link href="/formulaire?type=Proforma">
          <button style={{ ...inter, fontSize: 13, fontWeight: 700, padding: "8px 16px", borderRadius: 8, background: GREEN, color: ONYX, border: "none", cursor: "pointer" }}>+ Nouveau Proforma</button>
        </Link>
      </div>

      <div style={{ background: "#FFF", borderRadius: 14, border: "1px solid #E5E7EB", overflow: "hidden" }}>
        {loading ? <div style={{ padding: 32, textAlign: "center", ...inter, color: "#9CA3AF" }}>Chargement…</div>
          : paged.length === 0 ? <div style={{ padding: 40, textAlign: "center", ...inter, color: "#9CA3AF", fontSize: 14 }}>Aucun proforma trouvé.</div>
          : (
                ...
                {paged.map(d => {
                  ...
                  return (
                    <tr key={d.id} style={{ borderBottom: "1px solid #F3F4F6" }}
                      ...
                      <td style={{ ...inter, fontSize: 13, fontWeight: 600, padding: "12px 16px" }}>{d.numero}</td>
                      <td style={{ ...inter, fontSize: 13, color: "#374151", padding: "12px 16px" }}>{d.nom_client}</td>
                      <td style={{ ...inter, fontSize: 13, fontWeight: 600, padding: "12px 16px" }}>{fmt(d.montant_total)} {d.devise}</td>
